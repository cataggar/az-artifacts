# API and maintainer reference

[Back to the README](../README.md)

This reference covers publishing, downloading, discovery, metadata, file
...
- [Publishing](#publish-a-package)
- [Downloads and authentication](#download-a-package)
- [Discovery](#discover-feeds-packages-and-versions)
- [Metadata](#read-package-metadata)
...
### Protocol reference and live-publish gate

The [protocol fixtures](../tests/fixtures/publishing/protocol_evidence.json)
preserve normalized wire structures, not live identities or reusable approvals.
Opaque signatures and capabilities are not retained. The chunker is derived from
MIT-licensed [BuildXL][buildxl-hashing].

Publishing uses the canonical `A{instanceId}` dedup account path, negotiates
missing content and retention, and then registers the manifest and super-root.
Dedup HTTP 409 is negotiation, not a package conflict. Existing-node responses
may include immediate-child receipts, but the requested node must have its own
valid retention receipt; older child receipts never replace stronger proofs.
Registration uses the [public SDK's][upack-push-client] four
[fields][upack-push-model]; proofs are base64 serialized nodes, not content IDs.
