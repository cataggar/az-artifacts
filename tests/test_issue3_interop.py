    ...
    def __call__(self, request):
        ...
        if path.endswith("/Feeds"):
            ...
            return httpx.Response(
                ...
                json={
                    "count": 1,
                    "value": wire(self.baseline["cases"][20 if project else 0]["expected"]),
                },
            )
...
def execute(acceptance):
    ...
    code = issue3_readonly.run(
        root, config, baseline, output, transport=httpx.MockTransport(service)
    )
...
def test_compressed_response_decodes_once_preserving_other_header_semantics(status):
    ...
    original = httpx.Response(
        ...
        headers=[
            ...
            ("Operation-Location", "https://example.test/operation"),
    ...
    guard = support.GuardTransport(
        httpx.MockTransport(lambda _: original), support.Budget(issue3_example.LIMITS)
    )
    ...
    assert [
        (key, value) for key, value in response.headers.raw if key.lower() != b"content-length"
    ] == [
        (key, value)
        for key, value in original.headers.raw
...
def test_full_synthetic_matrix_and_explicit_unavailable_gaps(acceptance, capsys):
    ...
    assert all(
        record["operation"] == "resolver" for record in records if record["method"] == "POST"
    )
...
def test_inventory_without_manifest_requests_cannot_establish_coverage(acceptance, monkeypatch):
    ...
    monkeypatch.setattr(
        issue3_readonly.UniversalPackageClient,
        "list_files",
...
def test_digest_only_feed_oracle_preserves_all_data_without_recording_it(
    ...
    entry = {
        "request": case,
        "expected_count": len(feeds) + (change == "count"),
...
def test_deadline_prevents_next_request():
    ...
    with pytest.raises(support.Incomplete):
        support.GuardTransport(
            httpx.MockTransport(lambda _: pytest.fail("No network")), budget
        ).handle_request(httpx.Request("GET", "https://blob.example/x"))
...
def test_service_failure_is_not_absence(acceptance, status):
    ...
    assert (
        case_report(acceptance, "case-0004")["reason"]
        == {
...
def test_manifest_oracle_independently_distinguishes_manifest_leaves_and_file_payloads():
    ...
    oracle = support.ManifestOracle(baseline["manifests"][1], support.Budget(issue3_example.LIMITS))
    ...
    guard = support.GuardTransport(
        httpx.MockTransport(lambda _: pytest.fail("Payload blocked")),
        support.Budget(issue3_example.LIMITS),
    )
...
def test_arbitrary_writes_forbidden_before_transport(method):
    guard = support.GuardTransport(
        httpx.MockTransport(lambda _: pytest.fail("No write")),
        support.Budget(issue3_example.LIMITS),
    )
    with pytest.raises(support.BoundaryError):
...
def test_redirects_are_traced_and_bounded_without_persisting_urls():
    ...
    guard = support.GuardTransport(
        httpx.MockTransport(redirected), support.Budget({**issue3_example.LIMITS, "requests": 1})
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"receipts": {}},
        {"proofNodes": ["secret"]},
        {"url": "https://blob.example/a?sig=secret"},
...
def test_complete_matrix_can_pass_only_with_explicit_resource_fixtures(acceptance):
    ...
    def response(request):
        if "/fixture-denied/" in request.url.path:
            return httpx.Response(403)
        if (
            request.url.host == "feeds.dev.azure.com"
...
def test_corrupt_http_gzip_is_protocol_failure_not_missing_fixture(acceptance):
    ...
    def response(request):
        if request.url.path.endswith("/Feeds"):
            return httpx.Response(
                200, headers={"content-encoding": "gzip"}, stream=httpx.ByteStream(b"invalid-gzip")
            )
        return service(request)
...
def registration_proposal(config, baseline):
    ...
    files = [
        {
            ...
            "content_id": item["blob"]["id"],
            "size": item["blob"]["size"],
        }
        for item in json.loads(manifest_bytes)["items"]
    ]
    ...
    expected = {
        **baseline["cases"][4]["expected"],
        "version": "2.0.0-disposable.1",
    ...
    proposal = {
        **{
        ...
        "schema": 1,
        "kind": "issue3-registration",
        "fixture_only": False,
        "publisher": "registration-only",
        "approved": True,
        "disposable_version": True,
        "preuploaded_references_verified": True,
        "conflict_probe": False,
        "feed": "fixture-feed",
        "feed_id": issue3_example.FEED_ID,
        "scope": "organization",
        "name": "fixture-package",
        "version": expected["version"],
        "evidence_directory": "registration-evidence",
        "expected_metadata": expected,
        "proof_nodes_env": "AZ_ARTIFACTS_SYNTHETIC_PROOFS",
        "files": files,
        "proof_nodes_sha256": hashlib.sha256(
            json.dumps(proofs, separators=(",", ":"), ensure_ascii=True).encode("ascii")
        ).hexdigest(),
    }
...
def test_registration_only_one_put_and_public_readback(acceptance, monkeypatch, capsys):
    ...
    assert (
        issue3_registration.run(
            root, proposal, output, verified, transport=httpx.MockTransport(service)
        )
        == 0
    )
    ...
    with pytest.raises(FileExistsError):
        issue3_registration.run(
            root, proposal, new_output, verified, transport=httpx.MockTransport(service)
        )
...
def test_equivalent_destination_spellings_cannot_replay_registration(acceptance, field, spelling):
    ...
    if field == "project_id":
        proposal.update(
            scope="project", project="fixture-project", project_id=issue3_example.PROJECT_ID
        )
    service.registration_metadata = proposal["expected_metadata"]
    assert (
        issue3_registration.run(
    ...
    if field == "organization":
        ...
        proposal[field] = {
            "braces": "{" + identifier + "}",
            "hex": identifier.replace("-", ""),
            "uppercase": identifier.upper(),
            "urn": "urn:uuid:" + identifier,
        }[spelling]
    with pytest.raises(FileExistsError):
        issue3_registration.run(
            root,
...
def test_conflict_probe_requires_exact_approved_proposal(acceptance):
    ...
    assert (
        issue3_registration.run(
            root, proposal, output, tuple(proofs), transport=httpx.MockTransport(response)
        )
        == 0
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("approved", False),
        ("approved", 1),
        ("preuploaded_references_verified", False),
        ("disposable_version", False),
        ("publisher", "native-python"),
        ("completed", True),
        ("conflict_probe", "true"),
    ],
)
...
def test_registration_rejects_incomplete_content_proofs_before_any_attempt(acceptance, change):
    ...
    if change == "missing-content":
        ...
        proposal["files"].pop()
    proposal["proof_nodes_sha256"] = hashlib.sha256(
        json.dumps(
            proofs,
            separators=(",", ":"),
            ensure_ascii=True,
...
def test_registration_packed_content_tree_requires_all_intermediate_proofs(acceptance):
    ...
    root = registration_node(
        [
            (support.digest(packed, "02"), root_children[0][1]),
            root_children[1],
        ]
    )
...
def test_live_readonly_acceptance():
    code = issue3_readonly.cli(
        [
            "--private-directory",
            os.environ.get("AZ_ARTIFACTS_PRIVATE_DIRECTORY", ""),
            "--config",
            os.environ.get("AZ_ARTIFACTS_READONLY_CONFIG", ""),
            "--execute-readonly",
        ]
    )
    assert code == 0
...
def test_live_registration_only():
    code = issue3_registration.cli(
        [
            ...
            os.environ.get("AZ_ARTIFACTS_PRIVATE_DIRECTORY", ""),
            "--proposal",
            os.environ.get("AZ_ARTIFACTS_REGISTRATION_PROPOSAL", ""),
            "--proposal-sha256",
            os.environ.get("AZ_ARTIFACTS_REGISTRATION_PROPOSAL_SHA256", ""),
            "--execute-approved",
        ]
    )
    assert code == 0
